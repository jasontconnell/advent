...
func main() {
	...
	shop, err := common.ReadStrings(shopFilename)
	if err != nil {
		log.Fatal(err)
	}

	p1 := part1(input(in), shop)
	p2 := part2(input(in), shop)
...
func part1(in input, shop input) output {
	player := Player{Name: "Jason", Damage: 0, Defense: 0, HP: 100}
	w, _ := sim(player, getBoss(in), getShopItems(shop))
	return output(w[len(w)-1].GoldSpent)
}

func part2(in input, shop input) output {
	player := Player{Name: "Jason", Damage: 0, Defense: 0, HP: 100}
	_, l := sim(player, getBoss(in), getShopItems(shop))
	return output(l[len(l)-1].GoldSpent)
}

func sim(player Player, boss Player, shop []ShopItem) (VariationList, VariationList) {
...
func getShopItems(lines []string) []ShopItem {
	shop := []ShopItem{}
