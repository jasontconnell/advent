...
func main() {
	...
	// these values will just be in the code
...
func part1(in input) output {
	min := 1400
	hardMode := false
	res := RunSims(0, getSpells(), min, hardMode)

	sort.Slice(res, func(i, j int) bool {
		return res[i].Mana < res[j].Mana
	})

	return output(res[0].Mana)
}
...
func RunSims(threadID int, spells []Spell, min int, hardMode bool) []Result {
	minmana := min
	list := []Result{}
	for i := 0; i < 55000; i++ {
